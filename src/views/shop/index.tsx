        ...
        {currentData(data).map((product, index) => {
          if (Object.keys(product['filter'][0])[0] === 'color') {
            if (filterStore.filterOptions['Cores'].includes(product.filter[0].color) || filterStore.filterOptions['Cores'].length === 0) {
              return (
                <Product key={index} index={index} name={product.name} specialPrice={Object.keys(product).includes('specialPrice') ? product.specialPrice : undefined} price={product.price} imagePath={product.image} />
              );
            }
          }
            ...
            if (filterStore.filterOptions['Gênero'].includes(product.filter[0].gender) || filterStore.filterOptions['Gênero'].length === 0) {
              return (
                <Product key={index} index={index} name={product.name} specialPrice={400.00} price={product.price} imagePath={product.image} />
              );
            }
          }
          return null
